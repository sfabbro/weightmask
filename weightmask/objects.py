    ...
    if len(valid_data) > 1000:
        ...
        if mad_approx > 0:
            ...
            if tail_ratio > 3.0:
                ...
                print(
                    f"  [Adaptive SEP] Clutter penalty {clutter_penalty:.2f}x applied "
                    f"(tail ratio {tail_ratio:.1f}). Scaled extraction threshold to "
                    f"{extract_thresh:.2f} sigma."
                )
    return extract_thresh, m_in


def _filter_elongated_objects(objects, clean_config):
    if len(objects) == 0:
        return objects
    ...
    return objects[valid_obj]


def _apply_ellipse_mask(object_mask_ui8, objects, clean_config):
    ...
    if clean_config.get("dynamic_halo_scaling", True):
        print(
            f"    Halo scaling multiplier range: 1.0x to "
            f"{np.max(scale_multiplier):.2f}x"
        )
    return object_mask_ui8


def _apply_spike_mask(object_mask_ui8, objects, clean_config):
    if not clean_config.get("spike_enable", True):
        return object_mask_ui8
    ...
    if np.any(bright_mask):
        print(
            f"    Applying diffraction spike masking to {np.sum(bright_mask)} "
            f"bright stars (Flux > {spike_thresh:.2e})..."
        )
        ...
        for obj in objects[bright_mask]:
            ...
            s_len = int(
                spike_length_base * (1.0 + 0.2 * np.log10(obj["flux"] / spike_thresh))
            )
...
def detect_objects(data_sub, bkg_rms_map, existing_mask, config):
    ...
    try:
        ...
        extract_thresh, m_in = _calculate_adaptive_threshold(
            data_sub, bkg_rms_map, existing_mask, base_extract_thresh
        )
        ...
        objects = _filter_elongated_objects(objects, clean_config)

        print(f"  Detected {len(objects)} objects (thresh={extract_thresh:.1f} sigma).")

        if len(objects) > 0:
            object_mask_ui8 = _apply_ellipse_mask(
                object_mask_ui8, objects, clean_config
            )
            object_mask_ui8 = _apply_spike_mask(object_mask_ui8, objects, clean_config)
