...
def grow_bleed_trails(sci_data, sat_mask, sky_map, bkg_rms_map, config):
    ...
    if len(sat_cols) > 0:
        ...
        # Pre-allocate arrays and get config values outside the loop
        # to avoid redundant allocations and dictionary lookups
        bleed_thresh_sigma = config.get("bleed_thresh_sigma", 5.0)
        max_grow = config.get("bleed_grow_vertical", 50)

        if sky_map is None:
            sky_map_fallback = np.zeros(h)
        if bkg_rms_map is None:
            bkg_rms_map_fallback = np.full(h, 10.0)
        ...
        for s in slices:
            ...
            col_bkg = sky_map[:, x] if sky_map is not None else sky_map_fallback
            col_rms = bkg_rms_map[:, x] if bkg_rms_map is not None else bkg_rms_map_fallback
