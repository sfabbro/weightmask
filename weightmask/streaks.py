...
def _detect_streaks_frangi(data_sub, bkg_rms_map, existing_mask, config):
    ...
    sigmas = cfg.get(
        "sigmas", [1, 2, 3]
    )  # Scales for the ridge filter (determines streak width)
    ...
    high_threshold_sig = cfg.get(
        "high_threshold_sig", 5.0
    )  # Multiplier on median absolute deviation of frangi map
    ...
    try:
        ...
        if high_thresh is None or low_thresh is None:
            ...
            if np.sum(bkg_mask) < 1000:
                bkg_mask = np.ones_like(
                    tophat_img, dtype=bool
                )  # Fallback if entirely crowded
