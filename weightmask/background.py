        ...
        if mask_fraction > 0.8:
            ...
            try:
                ...
                if bkg.globalback == 0 and np.any(~mask):
                    ...
                    bkg_rms_map = np.full(
                        sci_data.shape, bkg.globalrms, dtype=np.float32
                    )
                ...
                method = (
                    "robust_median_fallback"  # Trigger the final fallback logic below
                )
        else:
            ...
            for attempt, scale in enumerate([1, 2, 4]):
                ...
                try:
                    ...
                    center_median = np.median(
                        bkg_map[edge_width:-edge_width, edge_width:-edge_width]
                    )
                    for i, edge_region in enumerate(edge_regions):
                        ...
                        if abs(edge_median - center_median) > 50:
                            ...
                            print(
                                "    Applied Gaussian smoothing to reduce edge artifacts"
                            )
                            break
                    break  # Success!
