...
def create_simulated_data(size=1024, noise_level=10.0, num_stars=50, streak_flux=30.0, regime_type='normal'):
    print(f"Generating synthetic data (size={size}, noise={noise_level}, stars={num_stars}, streak_flux={streak_flux}, regime_type={regime_type})...")
...
def evaluate_mask(pred_mask, gt_mask, name, pre_mask=None):
    ...
    print(f"  [{name}] Precision: {precision:.3f} | Recall: {recall:.3f} (TP:{true_positives}, FP:{false_positives}, FN:{false_negatives}, PredSum:{np.sum(p_mask)}, GTSum:{np.sum(g_mask)})")
...
def run_masking_test(config_path, args, save_fits=True):
    ...
    if save_fits: fitsio.write(os.path.join(output_dir, "mask_sat.fits"), sat_mask.astype(np.uint8), clobber=True)
    existing_mask |= sat_mask
    
    # 2. Test Cosmics
    avg_read_noise = float(np.median(bkg_rms))
    cr_mask = detect_cosmic_rays(sci_data, existing_mask, sat_level, 
                                gain=1.0, read_noise=avg_read_noise, 
    ...
    if save_fits: fitsio.write(os.path.join(output_dir, "mask_obj.fits"), obj_mask.astype(np.uint8), clobber=True)
    ...
    if save_fits: fitsio.write(os.path.join(output_dir, "mask_streak.fits"), streak_mask.astype(np.uint8), clobber=True)
...
def run_auto_sweep(report_file=None):
    ...
    regimes = [
        {"name": "Ideal (Low Noise, Sparse)", "size": 512, "noise": 5.0, "stars": 20, "streak": 50.0, "regime": "normal"},
        {"name": "Noisy Sparse (High Noise)", "size": 512, "noise": 25.0, "stars": 20, "streak": 25.0, "regime": "normal"},
        {"name": "Galactic Plane (Crowded)", "size": 512, "noise": 10.0, "stars": 200, "streak": 40.0, "regime": "normal"},
        {"name": "Ultra-Faint Artifacts", "size": 512, "noise": 15.0, "stars": 50, "streak": 15.0, "regime": "normal"},
        
        # Complex Regimes
        {"name": "Complex Ideal (Var Bkg/PSF)", "size": 512, "noise": 5.0, "stars": 20, "streak": 50.0, "regime": "complex"},
        {"name": "Complex Noisy", "size": 512, "noise": 25.0, "stars": 20, "streak": 25.0, "regime": "complex"},
        {"name": "Complex Crowded", "size": 512, "noise": 10.0, "stars": 200, "streak": 40.0, "regime": "complex"},
        
        # Extreme Edge Cases
        {"name": "Extreme Gradient Bkg", "size": 512, "noise": 10.0, "stars": 50, "streak": 30.0, "regime": "extreme_gradient"},
        {"name": "Elliptical PSF/Tracking err", "size": 512, "noise": 10.0, "stars": 50, "streak": 30.0, "regime": "elliptical_psf"},
        {"name": "Thick Satellite Streak", "size": 512, "noise": 10.0, "stars": 50, "streak": 100.0, "regime": "thick_streak"},
        {"name": "Extreme Poisson Crowded", "size": 512, "noise": 40.0, "stars": 300, "streak": 20.0, "regime": "complex"},
    ]
    
    results = {}
    for r in regimes:
        print(f"\n>>> REGIME: {r['name']}")
        print(f"Noise: {r['noise']} | Stars: {r['stars']} | Streak Flux: {r['streak']} | Regime: {r.get('regime', 'normal')}")
