...
def _add_saturated_stars(data, gt, size, x, y, regime_type):
    ...
    bleed_length = 60
    data[
        cy_sat - bleed_length : cy_sat + bleed_length, cx_sat - 1 : cx_sat + 2
    ] += flux_sat
...
def _benchmark_gate_failures(results):
    streak_f1 = {
        name: _f1(*metrics.get("Streaks", (0.0, 0.0)))
        for name, metrics in results.items()
    }
    cosmic_f1 = [
        _f1(*metrics.get("Cosmics", (0.0, 0.0))) for metrics in results.values()
    ]
    object_f1 = [
        _f1(*metrics.get("Objects", (0.0, 0.0))) for metrics in results.values()
    ]
    ...
    zero_failures = [
        name
        for name, metrics in results.items()
        if metrics.get("Streaks", (0.0, 0.0)) == (0.0, 0.0)
    ]
    if zero_failures:
        failures.append(
            "Zero streak precision/recall in: " + ", ".join(sorted(zero_failures))
        )
    ...
    crowded_object_recall = results.get("Galactic Plane (Crowded)", {}).get(
        "Objects", (0.0, 0.0)
    )[1]
    if crowded_object_recall < 0.95:
        failures.append(
            f"Crowded-field object recall {crowded_object_recall:.3f} < 0.950"
        )
