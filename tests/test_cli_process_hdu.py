...
class TestProcessHDU(unittest.TestCase):
    ...
    def test_process_hdu_happy_path(self, mock_generate_weight, mock_calc_inv_var,
                                   mock_detect_objs, mock_detect_crs, mock_est_bkg,
                                   mock_detect_sat, mock_detect_bad):
        # Setup mocks to return minimal expected values
        mock_detect_bad.return_value = np.zeros((100, 100), dtype=bool)
        mock_detect_sat.return_value = (65000.0, "mock", np.zeros((100, 100), dtype=bool))
        ...
        mask_data, inv_var_data, weight_map, confidence_map, sky_map, header_info = process_hdu(
            self.mock_hdu_sci, self.mock_hdu_flat, self.config, hdu_index=1, tile_size=100
        )
    ...
    @patch('weightmask.cli.calculate_inverse_variance')
    @patch('weightmask.cli.generate_weight_and_confidence')
    def test_process_hdu_no_flat(self, mock_generate_weight, mock_calc_inv_var,
                                mock_detect_objs, mock_detect_crs, mock_est_bkg,
                                mock_detect_sat, mock_detect_bad):
        # Setup mocks
        mock_detect_bad.return_value = np.zeros((100, 100), dtype=bool)
    ...
    def test_process_hdu_with_streaks(self, mock_detect_streaks, mock_generate_weight,
                                     mock_calc_inv_var, mock_detect_objs, mock_detect_crs,
                                     mock_est_bkg, mock_detect_sat, mock_detect_bad):
    ...
    def test_process_hdu_with_bleed_trails(self, mock_grow_bleed, mock_generate_weight,
                                          mock_calc_inv_var, mock_detect_objs, mock_detect_crs,
                                          mock_est_bkg, mock_detect_sat, mock_detect_bad):
    ...
    @patch('weightmask.cli.calculate_inverse_variance')
    @patch('weightmask.cli.generate_weight_and_confidence')
    def test_process_hdu_none_returns(self, mock_generate_weight, mock_calc_inv_var,
                                     mock_detect_objs, mock_detect_crs, mock_est_bkg,
                                     mock_detect_sat, mock_detect_bad):
        # Setup mocks to return minimal expected values
        mock_detect_bad.return_value = np.zeros((100, 100), dtype=bool)
